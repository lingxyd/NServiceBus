namespace NServiceBus
{
    ...
    class StartableEndpoint : IStartableEndpoint
    {
        ...
        public async Task<IStoppableEndpoint> StartAsync()
        {
            var sendOnlyBus = new ContextualBus(new BehaviorContextStacker(builder), builder.Build<IMessageMapper>(), builder, settings);
            var featureRunner = new FeatureRunner(builder, featureActivator);
            featureRunner.Start(sendOnlyBus);
            ...
            await runner.StartAsync(sendOnlyBus);
            ...
            var runningInstance = new UnicastBusInternal(builder, settings, pipelineCollection, runner, featureRunner);
            return await Task.FromResult<IStoppableEndpoint>(runningInstance);
        }
